...
{\normalsize\textbf{Fecha:} miércoles 5 de agosto de 2026}\\[0.2cm]
...
\textbf{Fecha y hora de la reunión:} miércoles 5 de agosto de 2026, de 08:00 a 09:30. \textbf{Duración:} {90 minutos}.
...
\begin{figure}
    \centering
    \includegraphics[width=0.82\linewidth]{Acta de la reunión de inspección formal 1.jpg}
    \caption{Acta de la reunión de inspección formal 1}
    \label{fig:acta-inspeccion-1}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.82\linewidth]{Acta de la reunión de inspección formal 2.jpg}
    \caption{Acta de la reunión de inspección formal 2}
    \label{fig:acta-inspeccion-2}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.82\linewidth]{Acta de la reunión de inspección formal 3.jpg}
    \caption{Acta de la reunión de inspección formal 3}
    \label{fig:acta-inspeccion-3}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=0.82\linewidth]{Acta de la reunión de inspección formal 4.jpg}
    \caption{Acta de la reunión de inspección formal 4}
    \label{fig:acta-inspeccion-4}
\end{figure}
...
\midrule
Mora Duarte Alex José & 15 & Listas de verificación, registro consolidado de defectos, formularios RFC y acta del CCB\\
Rizzo Vélez Edson Nagib & 23 & Estructura del repositorio, README, CHANGELOG, métricas, matriz de trazabilidad y evidencias de sesión\\
Villafuerte Rosero Allan Noe & 7 & ERS v1.0 y v1.1, documento 2A, lista de verificación del Inspector 1 y archivo de importación para Jira\\
\midrule
\textbf{Total} & 45 & Historial consultado en la rama \texttt{main} el 5 de agosto de 2026\\
\bottomrule
\end{tabular}
\end{table}
...
La etiqueta debe crearse sobre el commit que contiene el ERS v1.1 y publicarse en el repositorio remoto
antes de la entrega. Los comandos empleados son:
...
La comprobación final se realiza con \texttt{git tag -n} y
\texttt{git ls-remote --tags origin}; sus capturas forman parte del Anexo E.
...
\midrule
Mora Duarte Alex José & Preparación como Lector; lista de verificación individual; consolidación del registro de defectos; elaboración y versionado de RFC-01, RFC-02, RFC-03 y acta del CCB & 15\\
Rizzo Vélez Edson Nagib & Moderación e inspección; preparación de dos listas de verificación; métricas; estructura y documentación del repositorio; matriz de trazabilidad y evidencias de las sesiones & 23\\
Villafuerte Rosero Allan Noe & Inspección de RNF y modelado; publicación del ERS inspeccionado y corregido; revisión técnica de RFC-02; preparación del archivo de importación de Jira & 7\\
\bottomrule
\end{tabular}
\end{table}
...
Las listas de verificación se elaboraron en Microsoft Excel y permanecen en su formato original para
conservar las celdas, observaciones y metadatos de cada participante. Las cuatro copias se encuentran
en \path{02_Inspeccion/AnexoA_checklists/}:

\begin{table}[H]
\centering\footnotesize
\caption{Archivos que integran el Anexo A}
\begin{tabular}{L{4.2cm} L{3.3cm} L{7cm}}
\toprule
\textbf{Participante} & \textbf{Rol} & \textbf{Archivo}\\
\midrule
Mora Duarte Alex José & Lector & \path{AnexoA_Mora_Lector.xlsx}\\
Rizzo Vélez Edson Nagib & Moderador & \path{AnexoA_Rizzo_Moderador.xlsx}\\
Rizzo Vélez Edson Nagib & Inspector 2 & \path{AnexoA_Rizzo_Inspector2.xlsx}\\
Villafuerte Rosero Allan Noe & Inspector 1 & \path{AnexoA_Villafuerte_Inspector1.xlsx}\\
\bottomrule
\end{tabular}
\end{table}

\noindent
Los archivos pueden descargarse desde el directorio público del repositorio:
\url{https://github.com/erizzov-boop/ISR401-PFC-ERS-EQUIPO_B/tree/main/02_Inspeccion/AnexoA_checklists}.

\section{Registro de defectos}

El consolidado se reproduce en la Tabla~\ref{tab:defectos}. La hoja de cálculo original, con las
clasificaciones, severidades y correcciones, se encuentra en
\path{02_Inspeccion/AnexoB_registro_defectos.xlsx}.

\section{Formularios RFC y acta del CCB}

El Anexo C está compuesto por los tres formularios de solicitud de cambio y el acta firmada del
Change Control Board, disponibles en \path{03_CCB/}:

\begin{itemize}[leftmargin=*]
  \item \path{03_CCB/RFC-01.pdf}
  \item \path{03_CCB/RFC-02.pdf}
  \item \path{03_CCB/RFC-03.pdf}
  \item \path{03_CCB/Acta_CCB.pdf}
\end{itemize}

\section{Exportación CSV del backlog}

El repositorio contiene el archivo de carga \path{04_Trazabilidad/backlog_import_jira.csv}. La guía
exige, además, una exportación producida directamente por Jira con el nombre
\path{04_Trazabilidad/backlog_export.csv}. Al momento de cerrar este borrador, esa exportación todavía
no se encuentra versionada; debe generarse desde Jira e incorporarse antes de compilar la versión de
entrega del informe.

\section{Capturas del tablero y del repositorio}

El Anexo E debe incorporar evidencias propias, con el nombre del proyecto y la cuenta del integrante
visibles. Antes de la entrega deben añadirse a \path{04_Trazabilidad/capturas/} y
\path{06_Evidencias/capturas_git/} las siguientes imágenes:

\begin{enumerate}[leftmargin=*]
  \item tablero completo de Jira;
  \item detalle de al menos tres incidencias con los enlaces de trazabilidad visibles;
  \item panel de estadísticas y resultado del filtro de requisitos huérfanos;
  \item salida de \texttt{git log --oneline --graph --decorate --all};
  \item salida de \texttt{git tag -n} y evidencia de que \texttt{baseline-v1.1} está publicada.
\end{enumerate}

\noindent
Estas capturas no estaban disponibles en el repositorio consultado para esta revisión, por lo que no
se insertan imágenes ajenas ni se simula evidencia inexistente.
...
\midrule
ChatGPT (OpenAI) & GPT-5.6 Thinking, agosto de 2026 & Revisión técnica del archivo \LaTeX{}, corrección de errores de compilación, contraste con el repositorio y cierre de las tablas de autoría y anexos & Portada, \S8, \S12 y Anexos A--E & Los conteos de commits y los nombres de archivos se contrastaron con el historial y las rutas públicas del repositorio; no se inventaron capturas, exportaciones ni etiquetas ausentes\\
\midrule
Ninguna & --- & Métricas de inspección, decisiones del CCB y conclusiones críticas & \S4.5, \S6 y \S11 & Se derivan de artefactos propios del equipo (registro consolidado y acta de sesión) y se mantienen como producción del equipo\\
\bottomrule
\end{tabular}
\end{table}
